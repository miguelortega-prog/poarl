...
/**
 * Job para cargar archivo PAGPLA (Pagos Planilla) a base de datos.
 *
 * Carga solo la hoja correspondiente al periodo del run.
 * Este job se ejecuta en paralelo con otros jobs de carga.
 */
final class LoadPagplaDataSourceJob implements ShouldQueue
{
    use Batchable;
    use Dispatchable;
    ...
    /**
     * Tiempo máximo de ejecución (40 minutos - es el archivo más grande).
     */
     ...
     */
    private const PAGPLA_CODE = 'PAGPLA';

    /**
     * Tamaño de chunk optimizado para Excel muy grandes.
     */
    ...
    /**
     * Ejecuta el job de carga de PAGPLA.
     */
    public function handle(
        ...
        // Aumentar límite de memoria para archivos Excel muy grandes
        ini_set('memory_limit', '2048M');

        Log::info('Iniciando carga de PAGPLA a BD', [
            'job' => self::class,
            'run_id' => $this->runId,
            'period' => $this->period,
        ...
        try {
            ...
            // Buscar archivo PAGPLA
            $pagplaFile = null;
            foreach ($run->files as $file) {
                if (($file->dataSource->code ?? '') === self::PAGPLA_CODE) {
                    $pagplaFile = $file;
                    break;
                }
            }
            ...
            if ($pagplaFile === null) {
                throw new RuntimeException('Archivo PAGPLA no encontrado en el run');
            }

            $disk = $filesystem->disk('collection');
            $absolutePath = $disk->path($pagplaFile->path);

            if (!file_exists($absolutePath)) {
                throw new RuntimeException(
                    sprintf('Archivo PAGPLA no encontrado: %s', $absolutePath)
                );
            }

            Log::info('Archivo PAGPLA encontrado', [
                'run_id' => $run->id,
                'file_path' => $pagplaFile->path,
            ...
            Log::info('Hojas disponibles en PAGPLA', [
                'run_id' => $run->id,
                'sheets' => $sheetNames,
            ]);
            ...
            Log::info('Hoja de PAGPLA seleccionada', [
                'run_id' => $run->id,
            ...
            Log::info('Carga de PAGPLA completada exitosamente', [
                'job' => self::class,
                'run_id' => $run->id,
                'sheet_name' => $targetSheetName,
            ...
            Log::error('Error en carga de PAGPLA', [
                'job' => self::class,
                'run_id' => $this->runId,
                'period' => $this->period,
    ...
    private function determineTargetSheet(array $sheetNames, string $period): string
    {
        ...
        if ($period === '*') {
            throw new RuntimeException(
                'Periodo "*" (todos los periodos) no soportado en este job. Use LoadAllPagplaSheetsJob.'
            );
        }
        ...
        throw new RuntimeException(
            sprintf(
                'No se encontró una hoja en PAGPLA que contenga el año "%s". Hojas disponibles: %s',
                $year,
                implode(', ', $sheetNames)
            )
        );
    ...
    private function loadSheetToDb(
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            ...
            Log::info('Iniciando carga streaming de hoja PAGPLA', [
                'run_id' => $runId,
            ...
            foreach ($sheet->getRowIterator() as $row) {
                ...
                if (count($chunkData) >= self::CHUNK_SIZE) {
                    ...
                    if ((int) ($totalInserted / self::CHUNK_SIZE) % 5 === 0) {
                        Log::info('Progreso carga PAGPLA', [
                            'run_id' => $runId,
        ...
        Log::info('Hoja PAGPLA cargada a BD exitosamente', [
            'run_id' => $runId,
            'sheet_name' => $sheetName,
    ...
    {
        Log::error('Job de carga PAGPLA falló definitivamente', [
            'job' => self::class,
            'run_id' => $this->runId,
            'period' => $this->period,
            'error' => $exception->getMessage(),
